...
function RackTile({
  ...
  const isSimplified = renderHint?.simplify === true;
    ...
    >
      <div className="card__content">
        <div className={surfaceFrameClassName} {...rackGesture}>
          <div
            className={`card__surface card__surface--rack${tileMeta?.isRackDropTarget ? " card__surface--rack-target" : ""}${isSimplified ? " card__surface--rack-simplified" : ""}`}
            aria-label={`${card.title || "Rack"} with ${formatRackCount(card.tileIds?.length ?? 0)}`}
          >
            {isSimplified ? (
              <div className="card__rack-simplified-shell" aria-hidden="true" />
            ) : (
              <>
