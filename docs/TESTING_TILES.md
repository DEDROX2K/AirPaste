...
## Canvas Text QA Notes

For plain `canvas-text` coverage, verify:
...
- `Select`, `Hand`, and `Text` canvas tools
...
- right-click blank-canvas `Add text` creates a label at the menu position and focuses its editor
- right-clicking a plain text label exposes `Edit text`
- visible resize handle behavior on selected text boxes
- all text labels retain the shared fixed font and size
